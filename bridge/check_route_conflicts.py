#!/usr/bin/env python3
"""Check for cross-agent route conflicts before starting a managed tmux agent session."""
...
def main() -> int:
    ...
    parser.add_argument("--kind", required=True, choices=["codex", "claude"])
    parser.add_argument("--session", required=True)
    parser.add_argument("--chat-id", required=True)
    parser.add_argument("--channel", required=True)
    args = parser.parse_args()

    other_kind = "claude" if args.kind == "codex" else "codex"
    other_dir = CLAUDE_ROUTE_DIR if args.kind == "codex" else CODEX_ROUTE_DIR
    conflicts = [m for m in load_matches(other_dir, chat_id=args.chat_id, channel=args.channel) if m["active"] == "true"]

    if not conflicts:
        print(f"OK: no active {other_kind}-agent route conflicts for {args.channel}:{args.chat_id}")
        return 0

    print(
        f"CONFLICT: active {other_kind}-agent route already bound to {args.channel}:{args.chat_id}",
        file=sys.stderr,
    )
    ...
    print(
        f"Refusing to start {args.kind}-agent session {args.session} to avoid dual binding.",
        file=sys.stderr,
    )
    return 2
