...
当消息以 `>> ` 开头时，直接转发，不经过任何处理或指令解析：

1. 如果消息以 `>> ` 开头，去掉前缀
2. **立即转发，不做任何解释** - 不解析指令，不检查 /commands，不做任何改进
3. 将原始消息转发到绑定 session
4. 转发成功则回复 `已转发到 <session-name>`

**关键**：`>>` 前缀意味着"原样转发，不处理"。即使消息看起来像指令（如 `/clear`、`/help`），也要直接转发给 CLI agent。

### 示例
- `>> /clear` → 转发 "/clear" 到绑定 session（让 CLI agent 处理）
- `>> 帮我检查这段代码` → 原样转发（不改进）
