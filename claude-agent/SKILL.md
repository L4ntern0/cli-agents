...
- **自动审批**：print 模式用 `--dangerously-skip-permissions`；交互模式的 `start_claude.sh` 默认就是 `--auto`，Claude Code 自行决定执行，完成后通知我检查
- **我来审批**：显式传 `--approval`；Claude Code 遇到需确认的工具调用会暂停，pane monitor 唤醒我，我判断批准或拒绝，涛哥不需要介入
...
bash <skill_dir>/hooks/start_claude.sh claude-<name> <workdir>
...
**重要：session 级路由**
- 默认优先使用 `start_claude.sh` 传入 `--chat-id/--channel/--account/--agent`
- 脚本会为每个 session 写入 `/tmp/claude-agent-routes/<session>.json`
- `on_complete.py` 与 `pane_monitor.sh` 都按 route file 解析目标 thread / account / agent
- 不要再假设只改全局 `CLAUDE_AGENT_*` 就能正确覆盖多 session 并发场景
- `thread 通知` 与 `独立 thread 会话` 是两回事：前者由 hook 路由决定；后者还依赖 OpenClaw 的 Discord thread bindings 与该 thread 内真实入站消息触发
