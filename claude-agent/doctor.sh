...
PY
  ...
  echo "- no claude route files"
fi
echo

echo "[3/5] tmux sessions"
if tmux list-sessions 2>/dev/null | grep -q .; then
  tmux list-sessions | grep 'agentdeck\|claude' || echo "- no claude-like tmux sessions found"
else
  echo "- no tmux sessions"
fi
echo

echo "[4/5] claude notify log tail"
tail -n 20 /tmp/claude_notify_log.txt 2>/dev/null || echo "- no claude notify log yet"
echo
