...
- **Codex 自动审批**：`--full-auto`；`start_codex.sh` 默认就是这个模式，Codex 自行决定执行，完成后通知我检查
- **我来审批**：显式传 `--approval`；Codex 遇到需确认的操作会暂停，pane monitor 唤醒我，我判断批准或拒绝，涛哥不需要介入
...
**重要：session 级路由**
- 默认优先通过 `CODEX_AGENT_SOURCE_CHAT_ID / CHANNEL / ACCOUNT / AGENT_NAME` 启动
- `start_codex.sh` 会为每个 session 写入 `/tmp/codex-agent-routes/<session>.json`
- `on_complete.py` 与 `pane_monitor.sh` 都按 route file 解析目标 thread / account / agent
- 若未显式传入 `ACCOUNT`，Discord 可能回退到默认 bot 账号（例如 `Jarvis`），因此在多账号环境里要始终传入 `CODEX_AGENT_SOURCE_ACCOUNT`
- `thread 通知` 与 `独立 thread 会话` 是两回事：通知路由并不等于后续 `coding` agent 已绑定到该 thread
...
7. 在 OpenClaw 内启动 Codex 时，优先使用 `hooks/start_codex_openclaw.sh`，并显式传入当前消息来源（chat id / channel），不要默认依赖全局固定目标
