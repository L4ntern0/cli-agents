#!/usr/bin/env python3
"""Helpers for codex-agent per-session routing context."""
...
ROUTE_DIR = Path(os.environ.get("CODEX_AGENT_ROUTE_DIR", "/tmp/codex-agent-routes"))
...
def log_route_decision(event: str, **fields: Any) -> None:
    ...
    payload.setdefault("component", "codex-route-context")
...
def resolve_route(*, session_name: str | None = None, cwd: str | None = None, chat_id: str | None = None, channel: str | None = None) -> dict[str, str]:
    ...
    resolved = {
        "chat_id": str(route.get("chat_id") or os.environ.get("CODEX_AGENT_CHAT_ID") or "1476626965325807758"),
        "channel": str(route.get("channel") or os.environ.get("CODEX_AGENT_CHANNEL") or "discord"),
        "account": str(route.get("account") or os.environ.get("CODEX_AGENT_ACCOUNT") or ""),
        "agent_name": str(route.get("agent_name") or os.environ.get("CODEX_AGENT_NAME") or "main"),
        "session_name": str(route.get("session_name") or session_name or os.environ.get("CODEX_AGENT_SESSION") or ""),
