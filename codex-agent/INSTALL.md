...
# 方式 2：手动复制（如果你已经下载了）
cp -r /path/to/codex-agent ~/.openclaw/workspace-coding/skills/cli-agents/codex-agent
```

验证 skill 被识别：
```bash
ls ~/.openclaw/workspace-coding/skills/cli-agents/codex-agent/SKILL.md
# 应该存在
```
...
### 方式 B：session 级路由（推荐，适合 Discord thread / 多 session）
...
bash hooks/start_codex.sh codex-<name> /path/to/project --full-auto
```

这样 `start_codex.sh` 会写入 route file，之后：
- `on_complete.py` 完成通知
- `pane_monitor.sh` 审批通知
- `openclaw agent` 唤醒

都会沿用该 session 的 `chat_id/channel/account/agent_name`。

另外，`start_codex.sh` 现在默认就是**免审批启动**：
...
```bash
bash hooks/start_codex.sh codex-<name> /path/to/project --approval
```
...
```bash
cd ~/.openclaw/workspace-coding/skills/cli-agents/codex-agent/hooks/
...
openclaw agent --agent main --message "✅ codex-agent 唤醒测试" --channel telegram --timeout 10
...
1. 将 codex-agent skill 安装到 ~/.openclaw/workspace-coding/skills/cli-agents/codex-agent/
...
5. 设置脚本执行权限
6. 运行验证测试确认所有组件正常
安装指南在 skills/cli-agents/codex-agent/INSTALL.md
```
